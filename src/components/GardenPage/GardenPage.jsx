...
import BlueprintDisplay from '../BlueprintDisplay/BlueprintDisplay';

/**
 * GardenPage
 * This is the page for building the "Garden".
 * It combines the build area, palette, and blueprint.
 */
function GardenPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg shadow-md">
        ...
        <div className="h-64 bg-green-900/50 dark:bg-green-900/30 rounded-lg p-4 flex flex-wrap content-start gap-1 overflow-y-auto border-2 border-green-700 dark:border-green-600">
          {components.length === 0 && (
            <p className="text-gray-700 dark:text-gray-400 w-full text-center mt-4">
              Your garden is empty. Add some flowers and water!
            </p>
          )}
          {components.map((block) => (
            <div
              key={block.id}
              className={`w-6 h-6 rounded-full ${getBlockColor(block.type)} shadow-inner animate-flash`}
              title={block.type}
            />
          ))}
        </div>
